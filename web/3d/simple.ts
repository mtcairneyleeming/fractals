...
export class Simple3D {
    ...
    runN(scaleFactor: number, firstLayerDeltaZ: number, firstLayerLineLength: number, n: number, centre: boolean = false): Line3d[][] {
        // initial layer state
        let z = 0
        ...
        layers.push(this.drawLayer(this.axiom, new State(0, firstLayerLineLength / scaleFactor), z, 0, centre))

        let prevLayer = this.axiom

        for (let layerIndex = 1; layerIndex <= n; layerIndex++) {
            // update layer state
            z += firstLayerDeltaZ * Math.pow(scaleFactor, layerIndex - 2)
            console.log(z)
            ...
            let state = new State(layerIndex, firstLayerLineLength * currentScaleFactor / scaleFactor)
            let currLayer = ""
            ...
            for (let prevIndex = 0; prevIndex < prevLayer.length; prevIndex++) {
                ...
                for (let symbol of newSymbols) {
                    ...
                    let newLines = this.drawSymbol(state, symbol, currPos, prevIndex)
                    layer.push(...newLines)
     ...
     */
    drawLayer(str: string, state: State, z: number, prevIndex, centre: boolean): Line3d[] {
        let out = []
        let pos = new Point3d(0, 0, z)
        for (let symbol of str) {
            let lines = this.drawSymbol(state, symbol, pos, prevIndex)
            out.push(...lines)
            if (lines.length > 0) {
    ...
    drawSymbol(state: State, symbol: string, currPos: Point3d, prevIndex: number): Line3d[] {
        ...
        if (this.commands.has(symbol)) {
            ...
            let lines = svgToLines(commands, currPos.x, currPos.y)
            // then to 3d lines
